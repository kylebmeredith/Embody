...
1. **Prefer `.tdn` files for reading externalized COMPs** — every externalized COMP gets both `.tox` (binary, TD-native) and `.tdn` (JSON sidecar with complete network structure: operators, parameters, connections, positions, flags, DAT content, annotations). Reading the `.tdn` directly is faster than MCP round-trips. Call `get_externalizations` to list what's tracked. To edit: modify the `.tdn` on disk, then call `import_network` via MCP with the COMP path, the parsed JSON, and `clear_first=True` to reload it in TD. Use MCP when you need live runtime state (evaluated expressions, cook errors).
2. **Use Envoy MCP tools for live TD state and non-externalized operators** — NEVER say "I can't edit that because it's in a .tox" or "these are binary files I can't access." Use MCP tools to inspect and modify any operator. The filesystem holds externalized files; MCP is for live operator state inside TD.
...
5. **Discovery is par-driven** — a COMP is externalized iff `par.externaltox != ''`, a DAT iff `par.file != ''`. There is no tracking-file-on-disk; the in-memory Externalizations table is rebuilt from a live scan on every Update / Refresh. To externalize: set the par and pulse Update, or use Ctrl+W on the operator.
