...
- **Phase 1 — par-driven discovery.** `getOpsByPar` is now the canonical discovery function. A COMP is externalized iff `par.externaltox != ''` and a DAT is externalized iff `par.file != ''` (with a small DAT-type whitelist for safety). Clones, replicants, `/local`, and `engine`/`time`/`annotate` types are excluded via `isOpProcessable`. `Update()` drops the tag-intersection filter from both additions and subtractions. `isOpEligibleToBeExternalized` no longer requires a tag.
- **Phase 2 — flat paths + always-both output.** `getOpPaths` and `_buildTDNRelPath` emit `{folder}/{op.name}.{ext}`. No more mirroring of the network hierarchy on disk; the user's chosen folder is the file's home. Every TOX externalization now also writes a `.tdn` sidecar via the new `_writeTdnSidecar` helper -- called from `Save()` and `handleAddition` -- so every saved COMP has a diffable JSON view alongside its `.tox`. When the save path lands inside `app.userPaletteFolder`, `_setupCompForExternalization` writes `par.externaltox` as `app.userPaletteFolder + '/...'` (expression form) so the reference stays portable across machines. New public `TDNExt.RebuildToxFromTdn(tdn_path, target_comp_path)` for the "only the sidecar survived" recovery case.
- **Phase 3 — rip `externalizations.tsv` + Build/Date auto-injection.** The Externalizations table is no longer persisted to disk. It's now a pure in-memory view, rebuilt by `_scanAndPopulate()` on every `Update()` and `Refresh()`. An idempotent migration clears `par.file` on the table DAT and deletes the legacy `externalizations.tsv` on first scan. `handleAddition` / `handleSubtraction` no longer mutate the table inline -- their job is purely the file-side effect. `setupBuildParameters` body is a no-op (TODO: re-enable behind an opt-in setting); `Save()` / `SaveTDN()` Build/Date/Touchbuild bump blocks were commented out alongside.
- **Phase 5a — cleanup.** Deleted `getOpsToExternalize` (orphaned in Phase 1), `_reconstructAboutPage` and its single caller (was rebuilding About pages from the now-gone `.tsv`), and the entire rename-continuity subsystem in `EmbodyExt.py` (~960 lines): `checkOpsForContinuity`, `_checkExternalToxPar`, `_updateOpTimestamp`, `_findMovedOp`, `_findMovedTDNOp`, `_updateMovedTDNOp`, `_updateTDNChildren`, `_detectAncestorRename`, `_handleAncestorRename`, `_handleMissingOperator`, `_handleMissingOpsWithFiles`, `updateMovedOp`, `_removeOldFile`, `cleanupAllDuplicateRows`, `cleanupDuplicateRows`. TD natively preserves `par.externaltox` / `par.file` across renames, so the entire machinery built to keep a parallel tracking table aligned with live state is now dead weight. Duplicate detection (`checkForDuplicates` and friends) is retained -- it's a different concern. CLAUDE.md updated to describe par-driven discovery and flat layout.
- **Phase 5b.1 — tag system source cleanup.** Removed the entire tag-system surface in Python source (~39 KB / ~900 lines from `EmbodyExt.py`'s TAGGING UI section, plus scattered callsites): `getTags`, `applyTagToOperator`, `TagGetter`, `TagSetter`, `TagExiter`, `SetupTagger*`, `_cascadeTDNTag`, `_handleTDNAddition`, `_getTagColor`, `_setDATLanguageForTag`, `_inferDATTagValue`, `_getActiveDATTag`, `_removeTDNStrategy`, `_removeOrphanedTDNChildren`, `HandleStrategy*`, `HandleReload`, `_reloadTDN`, `_reloadTox`, `HandleEmbed`, `HandlePortableExport`, `HandleStrategyRemove`, `_removeExternalization`, `_dispatchTaggerButton`, `HandleDATConvert`, `ExternalizeImmediate`, `_setCloneTag`, `resetOpColor`, `_shouldDeleteFile`, `_alternateColor`. Callers updated: `getOpPaths` (DAT branch derives extension from `par.file`), `Disable` (signature drops `removeTags`), `ExportPortableTox` (no tag-strip phase), `_buildPathGroups` (par-only filtering), `RemoveListerRow`, `_handleDuplicateAsReference` (simplified to add 'clone' tag only), `ExternalizeProject` (sets pars directly instead of tagging), `_externalizeDATs`, `_findAtRiskDATs`, `RemoveTDNEntry` (clears `par.externaltox` + deletes `.tdn`), `ReconcileMetadata` (becomes a no-op -- the table is now derived from pars), `RestoreTOXComps` and `RestoreDATs` (drop the tag/color reapply steps), and `handleAddition` (no Tdntag routing). Module-level dicts collapsed: `dat_type_to_tag`, `extension_to_tag`, `tag_to_language`, `tag_to_extension` replaced with a single `dat_type_to_extension` map. `_PERSISTED_PARAMS` slimmed of tag/color params. Other files: `parexec.py` drops the `Tdncascade` handler and the `removeTags` kwarg; `keyboardin_callbacks.py` drops the lctrl-lctrl `TagGetter` shortcut; `chopexec_exit_tagger.py` is now an inert callback; `list/list_callbacks.py` shows blank instead of "Tag" for tree-mode parent rows and the strategy-cell click is a no-op until Phase 2.5 lands; `help/text_help.py` rewritten for par-driven discovery; `EnvoyExt.py` swaps `externalize_op(tag_type=...)` for `externalize_op(rel_path=...)` and renames `remove_externalization_tag` to `remove_externalization` (both par-driven); `TDNExt.py` retargets `_hasTDNTag` to a `par.externaltox` check, drops the `Tdncascade` warning-dialog flow, and `ExportProjectTDNInteractive` picks candidates from `par.externaltox`-set COMPs instead of tagged COMPs. Deleted test files: `test_tag_management.py`, `test_tag_lifecycle.py`, `test_strategy_handlers.py`. Remaining tag references all live in test files that will be updated in a follow-up sweep, plus `Embody.tdn` / `tagger.tdn` which Phase 5b.2 will rebuild via MCP.
- **Phase 5b.2 — TD UI cleanup.** Via Envoy MCP: deleted `/embody/Embody/tagger` (right-click tag-selection COMP, with descendants `tagger/header`, `tagger/header/text1`, `tagger/replicator1`), `/embody/Embody/window_tagging_menu` (the tagger popup), `/embody/Embody/annotation_tagging` (legacy annotation-tagging COMP), and `/embody/Embody/chopexec_exit_tagger`. Removed the entire `Tags` custom page from the Embody COMP (30 params: `Toxtag`, `Tdntag`, `Pytag`, `Csvtag`, `Dattag`, `Htmltag`, `Jsontag`, `Mdtag`, `Rtftag`, `Txttag`, `Xmltag`, `Glsltag`, `Fragtag`, `Verttag`, `Tsvtag`, plus the `*tagcolor*` RGBA groups for tox/dat/tdn/clone). Removed `Tdncascade` and `Tdncascadewarn` from the TDN page; removed `Addtagshort` and the `Taggingmenucolor*` group from the UI page. Net: -1278 lines after Embody.tdn re-export. Regression caught and fixed in the same commit: my Phase 5b.1 change to `TDNExt._hasTDNTag` (returning True for any par-set COMP) caused a parent TDN re-export to treat 33 legitimate child .tdn files as references and the stale-cleanup deleted them; restored them via git checkout and reverted `_hasTDNTag` to always return False so children always embed inline. Modular cross-file TDN references will return after the cleanup logic is rewritten to be aware of every tracked .tdn.
- **Phase 2.5 — Save UX + contextual action menu.** New methods on `EmbodyExt`: `OpenActionMenu(op_path=None)` opens a state-aware action menu for the operator under the cursor (or the supplied path). For an unexternalized op, the menu offers "Externalize (default folder)" (if `Defaulttoxfolder`/`Defaultscriptfolder` is set and exists) plus "Externalize (choose folder)...". For an already-externalized op, the menu offers Save, Reload from .tdn (COMPs only), and a "More..." sub-menu with Reveal-in-file-browser, Re-externalize, and Remove-externalization. `ReloadFromTdn(comp_path)` wraps `TDNExt.ImportNetworkFromFile(clear_first=True)` so the agent's edit-the-.tdn workflow has a one-call reload. `_getSaveLocation(target, use_default, is_tox)` resolves the externalization folder against the matching `Default*folder` param, falling back to `ui.chooseFolder` when the default is empty/missing. New custom params on the Embody COMP: `Defaulttoxfolder` and `Defaultscriptfolder` (Folder), `Synconsave` (Toggle, default True), `Actionmenushort` (Str, read-only label `'ctrl-w'`). `keyboardin_callbacks.py` binds `Ctrl+W` (no Shift, no Alt) to `OpenActionMenu()`. `execute.py:onProjectPreSave` now gates the pre-save `Update()` on `Synconsave` so the user can opt out of the full-save sync on every Ctrl+S. `_PERSISTED_PARAMS` updated. The action menu is a `ui.messageBox` cascade for the MVP (max 4 buttons per dialog); a custom popup panel can replace it in Phase 4 if the cascade feels clunky.
- **Phase 2.5 tweaks.** Action menu rewired from `ui.messageBox` to `op.TDResources.PopDialog.OpenDefault` (the bundled TD dialog Externalize uses): richer styling, Esc-on-click-away, keyboard Enter/Esc bindings. New `_popDialog(...)` helper centralises the call and falls back to `ui.messageBox` if PopDialog isn't available (e.g. headless tests). `Synconsave` default flipped to `False` -- most users keep externalization manual and don't want the full pre-save sync on every Ctrl+S.
- **Phase 4 — manager filters (in progress).** Two new toggles on the Embody COMP:
...
- **Phase 4 — Tree/flat list toggle.** New `Listmode` menu param on the Embody COMP, values `tree` / `flat`, default `flat`. In flat mode `inject_parents_callbacks.py` strips the synthetic ancestor rows after the text/state filters, and forces `depth=0` / `has_children=0` on every emitted row. Tree mode keeps the existing nested-with-expand-markers behavior. (Per-row Save / Reload cells render the same in both modes.)
- **Phase 4 — Rebuild all from .tdn.** New `Rebuildallfromtdn` pulse param on the Embody COMP. Calls a new `RebuildAllFromTdn()` method on EmbodyExt that walks every externalized COMP, computes the `.tdn` sidecar path via `_buildTDNRelPath`, and calls `TDNExt.ImportNetworkFromFile(clear_first=True)` for each one whose `.tdn` exists on disk. Skips COMPs whose `.tdn` is missing, logs per-COMP failures, and emits a summary at the end. Wired in `parexec.py:onPulse`. Use after `git pull`, fresh clone, or when in-TD state has drifted from on-disk JSON.
- **Phase 4 — `Saveallforce` skipped.** The plan called for a "re-save every externalized op regardless of dirty state" button. The existing `Update` already saves every dirty op, and the new per-row Save buttons cover the surgical case -- forcing a bulk re-save is paranoid noise we don't need.
