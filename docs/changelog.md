...
- **Phases 2.5, 4, and 5b pending.** 2.5 (default save folder + contextual action menu), 4 (tree/flat list toggle + dirty filter + TDN reload), and 5b (tag system removal -- `getTags`, `applyTagToOperator`, tagger UI, tag-related custom params, tag test files) need TD UI work via Envoy MCP and live in subsequent commits.
